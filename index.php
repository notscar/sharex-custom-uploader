...
global $emoji_list;
$length = 5;
$tokens = array("INSERT TOKEN HERE");
$emoji_list = [
...
function RandomEmojis($length,$emoji_list){

    $GENEMOJI = "";
    for($i=0; $i < $length; $i++) {
        $GENEMOJI .= $emoji_list[array_rand($emoji_list,1)];
    };
...
function GeneratePage($length, $file, $emoji_list) {
    $generatedEmojis = RandomEmojis($length,$emoji_list);
    $fileExtension = pathinfo($file['sharex']['name'], PATHINFO_EXTENSION);
    $tmpName = $file['sharex']['tmp_name'];
    $fileSize = $file['sharex']['size'] >> 10 ;
    $originalName = $file['sharex']['name'];
    $uploadTime = gmdate("Y-m-d H-i-s");
    $encEmojis = urlencode($generatedEmojis);
    mkdir($generatedEmojis ,0777,true);
    chmod($generatedEmojis,0777);
    ...
    <head>
        ...
        <meta property='og:image' content='/{$encEmojis}/image.png'/>
        <meta property='og:title' content='Scar Dumpster'>
        <meta property='og:description' content='{$fileSize} KB of wasted space.'/>
        <meta name='theme-color' content='#a705f7'>
        ...
        <style>
            ...
            figcaption {
                margin-top: 2.5vh;
                background: linear-gradient(90deg, rgb(162, 0, 255) 40%, rgba(142,209,252,1) 58%);
        ...
        <div class='container'>
            <div class='content'>
                <figure>
                    ...
                    <figcaption>{$uploadTime} UTC</figcaption>
                </figure>
    ...
    file_put_contents("{$generatedEmojis}/index.html",$page_content);
    if (move_uploaded_file($tmpName,"{$generatedEmojis}/image.png")) {
...
if(isset($_POST['secret_key'])) {
    ...
    if(in_array($_POST['secret_key'], $tokens)) {


        $result = GeneratePage($length,$_FILES,$emoji_list);

        if ($result) {

            $json = ['status' => 'OK','url' => $result];

        } else {

            $json = ['status' => 'ERROR','error' => "Couldn't Move File"];

        }

    } else {

        $json = ['status' => 'ERROR','error' => 'Authentication Error.'];
